...
class AnnuaireLDAP {
	...
	function __construct($admin, $pass) {
	...
	function get_users_info($uid='*'){
		...
		if ($uid == "admin") {
			return array(
				'cn' => "Helmut ARDELPIC",
				'Compte365' => 'admin@pdtor.dS3iose.a645caan5i4ule0ley',
				'uid' => "");
		}
	}
	...
	function get_usergroups( $grp ) {
		...
		for($i=0; $i<rand(1,35); $i++){
			...
			array_push($list, array(
				...
				'LastChange'		=> rand(0,10),
				'WhenCreated'		=> rand(0,10),
				));
		}
		sort($list);
		return $list;
	...
	function find_users($cherche, $non_utilise = false) {
		...
		$list = array();
		foreach( $users as $u ) {
			if( $cherche == null or stristr( $u['NomComplet'], $cherche ) != false ) {
				array_push ($list, array_change_key_case ($u, CASE_UPPER));
			}
		}
		return $list;
	}

...
sort($list);
$a = array("Reinitialiser","Masquer","Voir le mdp provisoire");
foreach ($list as $entry) {
	...
	if ($entry['logoncount'] == 0) {
		...
		echo "<span type='button' class='btn btn-outline-success btn-sm' 
				...
				data-bs-uid='{$entry['Identifiant']}' data-bs-name='{$entry['NomComplet']}'>
					Voir le mdp provisoire
			</span>";
	} else {
		...
		echo "<span type='button' class='btn btn-outline-success btn-sm' 
				...
				data-bs-uid='{$entry['Identifiant']}' data-bs-name='{$entry['NomComplet']}'>
					Reinitialiser avec un nouveau mot de passe
			</span>";
	}
...
function getMdp( elem ) {
	...
	if (btn_pwd.hasAttribute("enclair")) {
		...
		btn_pwd.innerHTML = 'Voir le mdp provisoire';		
		...
		// requete AJAX
		var myRequest = new Request('ajax.php?get=' + elem);
		fetch(myRequest).then(function(response) {
			return response.text().then(function(text) {
				if (text == '**************') {
					...
					btn_pwd.className = 'btn btn-outline-success btn-sm'
					btn_pwd.innerHTML = 'Reinitialiser avec un nouveau mot de passe';
...
function setMdp() {
	...
	if ((modalBodyPw.value.length >= 5) && (!btn_pwd.disabled)) {
		...
		btn_pwd.innerHTML = '<img src="runcat2.gif" width="76" height="32">&nbsp;travail en cours...';
		// requete AJAX
		var myRequest = new Request('ajax.php?set=' + modalBodyPw.value + '&uid=' + modalBodyId.value);
		fetch(myRequest).then(function(response) {
			return response.text().then(function(text) {
				if (text == 'OK') {
					// affichage
